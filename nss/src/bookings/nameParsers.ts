...
const PARSERS: Record<string, NameParser> = {
  ...
  // - Starts with optional '<*' then 'Block'
  ...
  // - Starts with * followed by a course code (4 letters + 4 digits)
  // - OR starts with FS- prefix
  // Examples:
  //   *MATH1141-T1U1-004
  //   *FINS5516-T1P1-001
  ...
  // Matches society bookings that are done internally via Arc
  // - Starts with * and an 8-digit date, then separator '-'
  // - ARCSG followed by name of society
  // - Numbers at the end for each distinct booking of soc
  // NOTE: Must come before SOCIETY since ARCSG is a more specific prefix of ARC
  // Examples:
  ...
  // - Starts with * and an 8-digit date, then separator '-'
  // - Followed by a name (not ARC/ARCSG, which are caught above)
  // Examples:
  ...
  // Other Misc bookings (catch-all)
  // - Two chars, then hyphen, then the name
  // - Or any remaining unmatched string
  // Examples:
