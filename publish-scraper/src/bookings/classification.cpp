...
model::BookingType classify_booking(const std::string_view event_type) {
    if (event_type == "Lecture" || event_type == "Lecture 1" || event_type == "Lecture 2") {
        return model::BookingType::Lecture;
    }

    if (event_type == "Tutorial" || event_type == "Tutorial 1" || event_type == "Tutorial 2") {
        return model::BookingType::Tutorial;
    }

    if (event_type == "Laboratory" || event_type == "Lab 1 of 2" || event_type == "Lab 2 of 2") {
    ...
    if (event_type == "Exam" || event_type == "*Exam Unit") {
        return model::BookingType::Exam;
    }

    if (event_type == "Clinical" || event_type == "Field" || event_type == "Honours" ||
        event_type == "Project" || event_type == "Thesis" || event_type == "Web" ||
        event_type == "Work" || event_type == "*Other class") {
        return model::BookingType::Class;
    }

    if (event_type == "*Booking" || event_type == "*Venues & Events" ||
        event_type == "BOOK.CONFIRMED" || event_type == "BOOK.REQUESTED" ||
        event_type == "BOOK.CANCELLED") {
        return model::BookingType::Miscellaneous;
    }

    if (event_type == "Other" || event_type == "*Not Used") {
...
model::BookingType classify_booking(const std::string_view event_type,
    ...
    const auto publish_type = classify_booking(event_type);
    ...
    const auto publish_is_specific = publish_type != model::BookingType::Miscellaneous &&
                                     publish_type != model::BookingType::Other &&
                                     publish_type != model::BookingType::Unknown;

    if (publish_is_specific) {
        return publish_type;
    }

    if (parsed_name.pattern == NamePattern::Class) {
        return model::BookingType::Class;
    }

    if (parsed_name.pattern == NamePattern::MiscClass) {
        return model::BookingType::Miscellaneous;
    }

    if (publish_type == model::BookingType::Unknown) {
...
std::string_view booking_type_name(const model::BookingType booking_type) {
    switch (booking_type) {
...
std::string_view booking_type_database_value(const model::BookingType booking_type) {
    switch (booking_type) {
    case model::BookingType::Lecture:
        return "LECTURE";
    ...
    case model::BookingType::Unknown:
        throw std::runtime_error{"Cannot serialize Unknown booking type"};
    }
