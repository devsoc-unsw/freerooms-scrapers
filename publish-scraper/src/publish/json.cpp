...
json serialize_date_period(
    const publish::DatePeriod& period
) {
    json result{
        {
            "Description",
...
json serialize_day(
    const publish::Day& day
) {
    return {
        {"Name", day.name},
        {"DayOfWeek", day.day_of_week},
        {"IsDefault", day.is_default}
    };
}

json serialize_time_period(
    const publish::TimePeriod& period
) {
    return {
        {
            "Description",
...
publish::Event parse_event(
    const json& value
) {
    publish::Event event;

    event.is_booking =
        value.at("IsBooking").get<bool>();

    event.is_currently_active =
        value.at("IsCurrentlyActive")
            .get<bool>();

    event.is_edited =
        value.at("IsEdited").get<bool>();

    event.is_deleted =
        value.at("IsDeleted").get<bool>();

    event.user_manually_added_event =
        value.at("UserManuallyAddedEvent")
            .get<bool>();

    event.event_identity =
        value.at("EventIdentity")
            .get<std::string>();
    ...
    event.start_date_time =
        value.at("StartDateTime")
            .get<std::string>();

    event.end_date_time =
        value.at("EndDateTime")
            .get<std::string>();
    ...
    event.is_published =
        value.at("IsPublished")
            .get<bool>();

    event.last_modified =
        value.at("LastModified")
            .get<std::string>();
    ...
    if (
        value.contains("ExtraProperties")
        && !value.at(
            "ExtraProperties"
        ).is_null()
    ) {
...
std::string serialize_events_request(
    ...
    root["CategoryTypesWithIdentities"] =
        json::array();

    for (
        const auto& selection :
        request.category_types_with_identities
    ) {
        root[
    ...
    view_options["DatePeriods"] =
        json::array();

    for (
        const auto& period :
        request.view_options.date_periods
    ) {
        view_options["DatePeriods"]
            .push_back(
                serialize_date_period(period)
            );
    }
    ...
    for (
        ...
        view_options["Days"].push_back(
            serialize_day(day)
        );
    }

    view_options["TimePeriods"] =
        json::array();

    for (
        const auto& period :
        request.view_options.time_periods
    ) {
        view_options["TimePeriods"]
            .push_back(
                serialize_time_period(period)
            );
    }
    ...
    for (
        const auto& week :
        request.view_options.weeks
    ) {
        view_options["Weeks"].push_back(
            serialize_week(week)
        );
    }

    root["FetchBookings"] =
        request.fetch_bookings;

    root["FetchPersonalEvents"] =
        request.fetch_personal_events;

    root["PersonalIdentities"] =
        request.personal_identities;
...
EventsResponse parse_events(
    ...
    try {
        ...
        throw std::runtime_error{
            "Invalid Publish events response: "
            + std::string{error.what()}
        };
    }
}
