...
std::size_t
write_callback(char* data, const std::size_t size, const std::size_t count, void* user_data) {
...
void append_header(curl_slist*& headers, const std::string& header) {
    auto* updated = curl_slist_append(headers, header.c_str());
...
Response Client::get(const std::string& url) {
    if (handle_ == nullptr) {
        throw std::runtime_error{"HTTP client has no curl handle"};
    }
    ...
    curl_easy_setopt(handle_, CURLOPT_ERRORBUFFER, error_buffer.data());

    curl_easy_setopt(handle_, CURLOPT_TIMEOUT, 30L);

    curl_easy_setopt(handle_, CURLOPT_USERAGENT, "publish-scraper/0.1");

    const auto result = curl_easy_perform(handle_);

    if (result != CURLE_OK) {
        const std::string message =
            error_buffer[0] != '\0' ? error_buffer.data() : curl_easy_strerror(result);
...
Response Client::post(const std::string& url) {
    ...
    curl_easy_setopt(handle_, CURLOPT_POSTFIELDSIZE, 0L);
...
Response Client::post_json(const std::string& url, const std::string& body) {
    return post_json(url, body, {});
}

Response Client::post_json(const std::string& url,
    ...
    curl_easy_setopt(handle_, CURLOPT_TIMEOUT, 600L);
