...
publish::Event make_room_details_event(
...
TEST_CASE(
    ...
    const auto lg01_event =
        make_room_details_event(
            room_details
        );

    const auto lg02_event =
        make_room_details_event(
            room_details
        );
    ...
    lg01_category.identity =
        "publish-lg01";

    lg01_category.name =
        "K-B16-LG01";

    lg01_category.results.push_back(
        lg01_event
    );

    publish::CategoryEvents lg02_category;

    lg02_category.identity =
        "publish-lg02";

    lg02_category.name =
        "K-B16-LG02";

    lg02_category.results.push_back(
        lg02_event
    );
    ...
    events.category_events.push_back(
        std::move(lg01_category)
    );

    events.category_events.push_back(
        std::move(lg02_category)
    );
