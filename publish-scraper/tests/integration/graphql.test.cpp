...
int scrape_year() {
    ...
    if (value == nullptr || *value == '\0') {
        throw std::runtime_error{"Integration tests require YEAR"};
    }
...
TEST_CASE("GraphQL exposes all buildings and rooms") {
    ...
    CHECK(data.at("buildings_aggregate").at("aggregate").at("count").get<int>() == 44);

    CHECK(data.at("rooms_aggregate").at("aggregate").at("count").get<int>() == 504);
}
...
TEST_CASE("GraphQL exposes room image URLs") {
    ...
    CHECK(room.at("id") == "K-B16-LG01");

    REQUIRE_FALSE(room.at("imageUrl").is_null());

    const auto image_url = room.at("imageUrl").get<std::string>();

    CHECK(image_url.starts_with("https://www.learningenvironments.unsw.edu.au/"));
}

TEST_CASE("GraphQL exposes bookings and room relationships") {
    ...
    const auto count = data.at("bookings_aggregate").at("aggregate").at("count").get<int>();
...
TEST_CASE("GraphQL exposes booking module relationships in both directions") {
    ...
    const auto& reverse_modules = booking.at("bookingmodules");

    REQUIRE_FALSE(reverse_modules.empty());
}